...
class SSD(nn.Module):
    def __init__(self, config, num_classes=28):
        ...
        # MobileNetV3 feature extraction points:
        # - layer 3: 24 channels (stride 8)
        # - layer 6: 40 channels (stride 16)
        # - layer 13: 160 channels (stride 32)
        # - final layer: 960 channels (stride 32)
        ...
        # Scale weight updated to match first feature map (24 channels)
        self.scale_weight = nn.Parameter(torch.ones(24) * 20)

        # Output channels updated for MobileNetV3
        out_channels = [24, 40, 160, 960, 512, 256]
    ...
    def forward(self, x, targets=None):
        # Feature extraction from MobileNetV3
        features = []
        
        # Forward pass through MobileNetV3 features
        for i, layer in enumerate(self.features):
            x = layer(x)
            # Capture feature maps at specific layers
            if i == 3:  # First feature map (stride 8) - 24 channels
                features.append(x)
            elif i == 6:  # Second feature map (stride 16) - 40 channels
                features.append(x)
            elif i == 13:  # Third feature map (stride 32) - 160 channels
                features.append(x)
        
        # Final features (stride 32) - 960 channels
        features.append(x)
        ...
        # Apply scale weight to the first feature map (24 channels)
...
def train_model():
    ...
    optimizer = torch.optim.SGD([
        {'params': model.features.parameters(), 'lr': config['train_params']['lr'] * 0.01},
    ...
    if os.path.exists(ckpt_path):
        ...
        # Optionally load optimizer state and epoch if continuing training
...
if __name__ == '__main__':
    ...
    infer_samples(num_samples=20)   # To run inference on sample images